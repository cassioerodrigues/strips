...
(function () {
  ...
  function correctPositions(store) {
    ...
    if (rootFamily) {
      ...
      if (parentFam && parentFam.children[0] && rootFamily.parents[0]) {
        var shift = getUnitX(parentFam, parentFam.children[0]) - getUnitX(rootFamily, rootFamily.parents[0]);
  ...
  function computeApiTreeLayout(people, unions, relationsByChild) {
    ...
    // Choose root
    var rootId = chooseRootId(rtNodes);
    ...
    if (unplacedNodes.length > 0) {
      ...
      while (unplacedNodes.length > 0) {
        ...
        // Generate links for this component
        generateLinksFromFamilies(componentResult.families, layout.nodes, unions, UNIT_X, UNIT_Y, layout.links);
    ...
    // Generate semantically-correct links from the family structure
    // so that tree.jsx can render union symbols, drops with fromUnion/toChild, and bus lines.
    generateLinksFromFamilies(result.families, layout.nodes, unions, UNIT_X, UNIT_Y, layout.links);
